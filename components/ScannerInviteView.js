...
import {
  scannerInviteSchemeUrl,
  IOS_APP_STORE_URL,
  ANDROID_PLAY_STORE_URL,
} from '../lib/app-links';

export default function ScannerInviteView({ token, preview }) {
  ...
  const openInApp = useCallback(() => {
    ...
    window.location.href = scannerInviteSchemeUrl(token);
      ...
      <div className="tsx-card">
        ...
        <div className="tsx-body">
          <div className="tsx-brand">
            <SambaLogo size={28} />
            <span>Scanner invite</span>
          </div>

          <h1 className="tsx-title">{preview.eventTitle}</h1>

          {preview.organizerName && (
            <p className="tsx-hosted">Hosted by {preview.organizerName}</p>
          )}

          {dateLabel && <p className="tsx-date">{dateLabel}</p>}
          {preview.eventVenue && <p className="tsx-venue">{preview.eventVenue}</p>}

          <p className="tsx-message">
            Join as a ticket scanner for this event
          </p>
