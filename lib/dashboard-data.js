...
import { buildEventInsertRow, buildEventUpdateRow, validateCreateEventPayload } from './create-event';
...
function buildTrends(tickets, days = 30) {
  ...
  tickets.forEach((t) => {
    const raw = t.purchase_date || t.created_at;
    if (!raw) return;
...
export async function fetchOrganizerDashboardData(organizerId, db) {
  ...
  return {
    ...
    tickets: ticketRows.map((t) => ({
      ...
      status: t.status,
      purchaseDate: t.purchase_date || t.created_at || null,
...
export async function fetchPlatformAdminData() {
  ...
  const [{ data: profiles }, { data: events }, { data: tickets }] = await Promise.all([
    ...
    admin.from('tickets').select('*').order('id', { ascending: false }),
  ]);
  ...
  const organizerStats = organizers.map((org) => {
    ...
    return {
      ...
      avatar: org.provider_info?.partyLogo || org.profile_picture_url || null,
...
async function assertOrganizerOwnsEvent(client, organizerId, eventId) {
...
export async function fetchOrganizerEventDetail(organizerId, eventId, db) {
  const client = db || createAdminClient();
  if (!client) throw new Error('Server misconfigured');

  const row = await assertOrganizerOwnsEvent(client, organizerId, eventId);
  const event = mapEventRow(row);
  const revenue = eventRevenue(event);
...
export async function updateEvent(organizerId, eventId, updates, db) {
  ...
  const existing = await assertOrganizerOwnsEvent(client, organizerId, eventId);
  ...
  if (updates.status === 'live' && existing.status !== 'live') {
    allowed.live_started_at = new Date().toISOString();
    allowed.started_early = true;
  }
  if (updates.status === 'completed') {
    allowed.end_time = allowed.end_time || new Date().toISOString();
  }

  if (updates.fullEdit) {
    const errors = validateCreateEventPayload(updates.fullEdit);
    if (errors.length) throw new Error(`Missing: ${errors.join(', ')}`);
    Object.assign(allowed, buildEventUpdateRow(updates.fullEdit, existing));
  }
