...
export async function fetchOrganizerDashboardData(organizerId) {
  const admin = createAdminClient();
  if (!admin) throw new Error('Server misconfigured');

  const [{ data: profile }, { data: events }] = await Promise.all([
    admin.from('profiles').select('*').eq('id', organizerId).single(),
    admin.from('events').select('*').eq('organizer_id', organizerId).order('date', { ascending: false }),
  ]);
...
export async function updateEvent(organizerId, eventId, updates) {
  const admin = createAdminClient();
  if (!admin) throw new Error('Server misconfigured');
